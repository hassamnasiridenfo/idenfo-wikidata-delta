...
def normalize_list_strings(
    ...
    For example '['uae'],['uk']' -> 'uae, UK' or ['uae', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches uae_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General United Arab Emirates"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["ae_gen"] * num_rows,
            "Updated On": [""] * num_rows,
            "Added On": ["2026-01-14"] * num_rows,
            "Status": [1] * num_rows,
            "Charges": [""] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "AE"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match uae_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches uae_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    for i, row in raw_df.iterrows():
        ...
        try:
            ...
            clean_df.loc[i, "Name"] = (
                "Tawfiq Abdul Hay"
                if original_id == "Q12203473"
                else name_data.get("name", "")
            )
    ...
    if rca_records:
        ...
        for col in clean_df.columns:
            if col not in rca_df_clean.columns:
                if col in list_columns:
                    ...
                    rca_df_clean[col] = "General United Arab Emirates"
                elif col in ["List Type"]:
                    rca_df_clean[col] = "Individual"
                elif col in ["Scraper Tag"]:
                    rca_df_clean[col] = "ae_gen"
                elif col in ["Added On"]:
                    rca_df_clean[col] = "2026-01-14"
                else:
                    rca_df_clean[col] = ""
        ...
        clean_df.to_excel(
            os.path.join(CLEANED_DIR, CLEAN_FILE_PATH),
            index=False,
        )
...
def uae_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame:
    global RAW_FILE_PATH                    # the variable declared at top of file 
    if raw_file_path is not None: 
        RAW_FILE_PATH = raw_file_path 
    try:
        logger.info("Starting uae PEP scraper...")
        clean_df = get_clean_df()
        clean_df = common_cleaning(clean_df)
        clean_df = replacements_for_delta(clean_df)
        logger.info("uae PEP scraper completed successfully.")
        return clean_df
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "uae_pep_scrapper()", e, "Error in uae PEP scraper"
            )
        )
        raise
