...
RCA_FILE_PATH = os.path.join(
    CLEANED_DIR, "pep_lithuania_living_relevant_rca_lookup.xlsx"
)
...
def normalize_list_strings(
    ...
    For example '['lithuania'],['uk']' -> 'lithuania, UK' or ['lithuania', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches lithuania_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Lithuania"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["lt_gen"] * num_rows,
            "Updated On": [""] * num_rows,
            "Added On": ["2026-01-12"] * num_rows,
            "Status": [1] * num_rows,
            "Charges": [""] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "LT"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match lithuania_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches lithuania_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    if rca_records:
        ...
        clean_df.to_excel(
            os.path.join(CLEANED_DIR, "pep_lithuania_living_relevant_cleaned.xlsx"),
            index=False,
        )
...
def lithuania_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    ...
    try: 
        ...
        logger.error(
            get_standard_logger_message(
                "lithuania_pep_scrapper()", e, "Error in lithuania PEP scraper"
            )
        )
        raise
