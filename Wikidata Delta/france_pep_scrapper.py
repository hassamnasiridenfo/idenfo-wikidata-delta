...
METADATA = {
    "Name": "General France",
    "Publisher": "Idenfo",
    "Description": "A detailed database of France's political figures and leadership profiles, offering reliable information for compliance, due diligence, and risk management. Designed for financial institutions, corporates, and professionals who require accurate insights into France's political landscape.",
    "Country": "FR",
...
RAW_DIR = BASE_DIR / "fr_gen_excels"
CLEANED_DIR = BASE_DIR / "fr_gen_excels"
UTILS_DIR = BASE_DIR
...
CLEAN_XLSX   = os.path.join(CLEANED_DIR, "pep_france_living_relevant_clean.xlsx")
...
LOG_FILE = BASE_DIR / "fr_gen_excels"/ "fr_pep_gen.log"
...
def normalize_list_strings(
    ...
    For example '['france'],['uk']' -> 'france, UK' or ['france', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches france_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General France"] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "FR"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match france_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches france_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
        ...
        for col in clean_df.columns:
            if col not in rca_df_clean.columns:
                if col in list_columns:
                    ...
                    rca_df_clean[col] = "General France"
    ...
    clean_df.to_excel(
        os.path.join(CLEANED_DIR, "pep_france_living_relevant_cleaned.xlsx"),
        index=False,
    )
...
def france_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
        try:
            ...
            logger.error(
                get_standard_logger_message(
                    "france_pep_scrapper()", e, "Error in france PEP scraper"
                )
            )
            raise
