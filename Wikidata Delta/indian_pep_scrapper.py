...
import string
import unicodedata
from structured_scraping.wikidata.final_format import pep_to_ireland44
...
METADATA = {
    "Name": "General India",
    "Publisher": "Idenfo",
    "Description": "A detailed database of India's political figures and leadership profiles, offering reliable information for compliance, due diligence, and risk management. Designed for financial institutions, corporates, and professionals who require accurate insights into India's political landscape.",
    "Country": "IN",
...
RAW_DIR = SCRAPER_DIR / "in_gen_excels"
CLEANED_DIR = SCRAPER_DIR / "in_gen_excels"
...
TRANSLATION_CACHE_PATH = os.path.join(CLEANED_DIR, "pep_india_translation_cache.xlsx")
...
CLEAN_FILE_PATH = os.path.join(CLEANED_DIR, "pep_india_living_relevant_cleaned.xlsx")
RAW_FILE_PATH = os.path.join(RAW_DIR, "pep_india_living_relevant_raw.xlsx")
RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_india_living_relevant_rca_lookup.xlsx")
...
def normalize_list_strings(
    ...
    For example '['india'],['uk']' -> 'india, UK' or ['india', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches india_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General India"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["in_gen"] * num_rows,
            "Updated On": [""] * num_rows,
...
def get_gender(genderLabel: str) -> str:
    ...
    if gender_lower in other_genders:
        return "Other"
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "IN"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match india_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches india_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    # Track rows to remove (those with Q-code names and no aliases)
    rows_to_remove = []
    ...
    for i, row in raw_df.iterrows():
        ...
        try:
            ...
            # Filter out Q-code addresses
            addresses = parse_list_entries(row.get("residenceLabel", ""))
            filtered_addresses = [
                addr for addr in addresses if not re.match(r"^Q\d+$", addr.strip())
            ]
            clean_df.at[i, "Primary Address"] = filtered_addresses
            ...
            # Filter out Q-code aliases
            unique_aliases = []
            unique_alias_types = []
            for a, at in zip(aliases, alias_types, strict=False):
                # Skip Q-code aliases
                if re.match(r"^Q\d+$", a.strip()):
                    continue
            ...
            # Check if name is Q-code and no aliases - mark for removal
            if (
                re.match(r"^Q\d+$", clean_df.loc[i, "Name"].strip())
                and len(unique_aliases) == 0
            ):
    ...
    # Remove rows with empty names (after trying to fill from aliases)
    clean_df = clean_df[clean_df["Name"] != ""].reset_index(drop=True)

    # Remove rows with Q-code names and no aliases
    if rows_to_remove:
        logger.info(
            f"Removing {len(rows_to_remove)} records with Q-code names and no aliases"
        )
        clean_df = clean_df.drop(rows_to_remove).reset_index(drop=True)
    ...
    clean_df.to_excel(
        os.path.join(CLEANED_DIR, "pep_india_living_relevant_cleaned.xlsx"),
        index=False,
    )
...
def india_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    ...
    try: 
        logger.info('Starting india PEP scraper...')
        ...
        logger.info("india PEP scraper completed successfully.")
        return clean_df
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "india_pep_scrapper()", e, "Error in india PEP scraper"
            )
        )
        raise
