...
RAW_DIR = BASE_DIR / "bh_gen_excels"
CLEANED_DIR = BASE_DIR / "bh_gen_excels"
UTILS_DIR = BASE_DIR
...
CLEAN_XLSX   = os.path.join(CLEANED_DIR, "pep_bahrain_living_relevant_clean.xlsx")
...
def normalize_list_strings(
    ...
    For example '['bahrain'],['uk']' -> 'bahrain, UK' or ['bahrain', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches bahrain_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Bahrain"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["bh_gen"] * num_rows,
            "Updated On": [""] * num_rows,
            "Added On": ["2025-12-24"] * num_rows,
            "Status": [1] * num_rows,
            "Charges": [""] * num_rows,
    ...
    try:
        if raw_file_path is None:
            raw_file_path = os.path.join(
                RAW_DIR, "pep_bahrain_living_relevant_raw.xlsx"
            )
        return pd.read_excel(raw_file_path, sheet_name=sheet, engine="openpyxl")
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "get_sheet_df()", e, f"{sheet} does not exist in raw file"
            )
        )
        return pd.DataFrame()
...
def get_place_of_birth(birthPlaceLabel: str, extra_info: dict) -> str:
    ...
    if birthPlaceLabel.count("]") == 1 and birthPlaceLabel.count("[") == 1:
        ...
        places = [p for p in places if not check_if_date(p)]
        if len(places) == 0:
            return ""
        if len(places) > 1:
...
def get_role_details(
    ...
    # urls can be capital as well Http ? So using re.IGNORECASE
    designations = [
        d for d in designations if re.search(r"https?:\/\/", d, re.IGNORECASE) is None
    ]
...
def get_father_name(fatherLabel: str) -> str:
    ...
    strs_to_rem.sort(key=len, reverse=True)
    for s in strs_to_rem:
        if s.lower() in father_name.lower():
            father_name = father_name.lower().replace(s.lower(), "").strip().title()

    father_name = (
        father_name.replace("‘", " ").replace("-", " ").replace("&", " and ").strip()
    )
...
def get_primary_occupation(occupationLabel: str) -> list[str]:
    if pd.isna(occupationLabel) or not occupationLabel:
        return []
    occupations = normalize_list_strings(
        occupationLabel, return_type="list", splitter=","
    )
    occupations = [
        occ.strip() for occ in occupations if not re.search(r"https?:\/\/", occ)
    ]
    occupations = [
        occ.strip().title() for occ in occupations if not re.search(r"^Q\d+$", occ)
    ]
...
def get_clean_df() -> pd.DataFrame:
    ...
    for i, row in raw_df.iterrows():
        try:
            ...
            clean_df.at[i, "Primary Occupation"] = get_primary_occupation(
                row.get("occupationLabel", "")
            )
            (
    ...
    if rca_records:
        ...
        clean_df.to_excel(
            os.path.join(CLEANED_DIR, "pep_bahrain_living_relevant_cleaned.xlsx"),
            index=False,
        )
...
def bahrain_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    global RAW_FILE_PATH                    # the variable declared at top of file 
    if raw_file_path is not None: 
        RAW_FILE_PATH = raw_file_path 
    try:
        logger.info("Starting bahrain PEP scraper...")
        ...
        logger.error(
            get_standard_logger_message(
                "bahrain_pep_scrapper()", e, "Error in bahrain PEP scraper"
            )
        )
        raise