...
RAW_DIR = BASE_DIR / "qa_gen_excels"
CLEANED_DIR = BASE_DIR / "qa_gen_excels"
UTILS_DIR = BASE_DIR
...
# PATHS
RAW_FILE_PATH = os.path.join(RAW_DIR, "pep_qatar_living_relevant_raw.xlsx")
CLEAN_FILE_PATH = os.path.join(CLEANED_DIR, "pep_qatar_living_relevant_cleaned.xlsx")
RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_qatar_living_relevant_rca_lookup.xlsx")

LOG_FILE = BASE_DIR / "qa_gen_excels" / "qa_pep_gen.log"

logger = logging.getLogger("qatarPEPScrapper")
...
def normalize_list_strings(
    ...
    For example '['qatar'],['uk']' -> 'qatar, UK' or ['qatar', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches qatar_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Qatar"] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "QA"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match qatar_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches qatar_pep_scrapper.py"""
        ...
        for col in clean_df.columns:
            if col not in rca_df_clean.columns:
                if col in list_columns:
                    ...
                    rca_df_clean[col] = "General Qatar"
...
def qatar_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    ...
    try: 
        logger.info('Starting Qatar PEP scraper...') 

        clean_df = get_clean_df()  
        clean_df = common_cleaning(clean_df)
        clean_df = replacements_for_delta(clean_df)
        logger.info("Qatar PEP scraper completed successfully.")
        return clean_df
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "qatar_pep_scrapper()", e, "Error in qatar PEP scraper"
            )
        )
        raise