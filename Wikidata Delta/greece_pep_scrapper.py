...
# RAW_DIR = BASE_DIR
# CLEAN_FILE_PATH = os.path.join(CLEANED_DIR, "pep_greece_living_relevant_cleaned.xlsx")
# RAW_FILE_PATH = os.path.join(RAW_DIR, "pep_greece_living_relevant_raw.xlsx")
# RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_greece_living_relevant_rca_lookup.xlsx")

BASE_DIR = Path(__file__).parent
RAW_DIR = BASE_DIR / "gr_gen_excels"
CLEANED_DIR = BASE_DIR / "gr_gen_excels"
UTILS_DIR = BASE_DIR
...
#  PATHS
RAW_FILE_PATH     = os.path.join(RAW_DIR, "pep_greece_living_relevant_raw.xlsx")
CLEAN_XLSX   = os.path.join(CLEANED_DIR, "pep_greece_living_relevant_clean.xlsx")
RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_greece_living_relevant_rca_lookup.xlsx")
COUNTRY_FILE = os.path.join(UTILS_DIR, "Updated CountryList.xlsx")
LOG_FILE = BASE_DIR / "gr_gen_excels"/ "gr_pep_gen.log"
...
def normalize_list_strings(
    ...
    For example '['greece'],['uk']' -> 'greece, UK' or ['greece', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches greece_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Greece"] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "GR"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match greece_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches greece_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
        ...
        for col in clean_df.columns:
            if col not in rca_df_clean.columns:
                if col in list_columns:
                    ...
                    rca_df_clean[col] = "General Greece"
    ...
    clean_df.to_excel(
        os.path.join(CLEANED_DIR, "pep_greece_living_relevant_cleaned.xlsx"),
        index=False,
    )
...
def greece_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    global RAW_FILE_PATH                    # the variable declared at top of file 
    if raw_file_path is not None: 
        RAW_FILE_PATH = raw_file_path 
    try:
        logger.info("Starting greece PEP scraper...")
        ...
        logger.error(
            get_standard_logger_message(
                "greece_pep_scrapper()", e, "Error in greece PEP scraper"
            )
        )
        raise
