...
RAW_FILE_PATH = os.path.join(RAW_DIR, "pep_switzerland_living_relevant_raw.xlsx")
RCA_FILE_PATH = os.path.join(
    CLEANED_DIR, "pep_switzerland_living_relevant_rca_lookup.xlsx"
)
...
def normalize_list_strings(
    ...
    For example '['switzerland'],['uk']' -> 'switzerland, UK' or ['switzerland', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches switzerland_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Switzerland"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["ch_gen"] * num_rows,
            "Updated On": [""] * num_rows,
            "Added On": ["2026-01-31"] * num_rows,
            "Status": [1] * num_rows,
            "Charges": [""] * num_rows,
...
def get_gender(genderLabel: str) -> str:
    ...
    gender = genders[0].lower()
    if gender in ["trans woman", "trans man", "non-binary"]:
        return "Other"
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "CH"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match switzerland_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches switzerland_pep_scrapper.py"""
...
def switzerland_pep_scrapper() -> pd.DataFrame:
    try:
        ...
        logger.error(
            get_standard_logger_message(
                "switzerland_pep_scrapper()", e, "Error in switzerland PEP scraper"
            )
        )
        raise
