...
# # RAW_DIR = BASE_DIR
# CLEAN_FILE_PATH = os.path.join(CLEANED_DIR, "pep_uzbekistan_living_relevant_cleaned.xlsx")
# RAW_FILE_PATH = os.path.join(RAW_DIR, "pep_uzbekistan_living_relevant_raw.xlsx")
# RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_uzbekistan_living_relevant_rca_lookup.xlsx")

BASE_DIR = Path(__file__).parent
RAW_DIR = BASE_DIR / "uz_gen_excels"
CLEANED_DIR = BASE_DIR / "uz_gen_excels"
UTILS_DIR = BASE_DIR
...
LOG_FILE = BASE_DIR / "uz_gen_excels"/ "uzbekistan_pep.log"
...
def normalize_list_strings(
    ...
    For example '['uzbekistan'],['uk']' -> 'uzbekistan, UK' or ['uzbekistan', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches uzbekistan_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Uzbekistan"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["uz_gen"] * num_rows,
            "Updated On": [""] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "UZ"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match uzbekistan_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches uzbekistan_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    if rca_records:
        ...
        clean_df.to_excel(
            os.path.join(CLEANED_DIR, "pep_uzbekistan_living_relevant_cleaned.xlsx"),
            index=False,
        )
...
def uzbekistan_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 

    global RAW_FILE_PATH                    # the variable declared at top of file 
    if raw_file_path is not None: 
        RAW_FILE_PATH = raw_file_path 
    try:
        logger.info("Starting uzbekistan PEP scraper...")
        clean_df = get_clean_df()
        clean_df = common_cleaning(clean_df)
        clean_df = replacements_for_delta(clean_df)
        logger.info("uzbekistan PEP scraper completed successfully.")
        return clean_df
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "uzbekistan_pep_scrapper()", e, "Error in uzbekistan PEP scraper"
            )
        )
        raise
