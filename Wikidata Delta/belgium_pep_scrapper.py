...
CLEANED_DIR = BASE_DIR / "be_gen_excels"
UTILS_DIR = BASE_DIR
...
#  PATHS
RAW_FILE_PATH     = os.path.join(RAW_DIR, "pep_belgium_living_relevant_raw.xlsx")
CLEAN_XLSX   = os.path.join(CLEANED_DIR, "pep_belgium_living_relevant_clean.xlsx")
RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_belgium_living_relevant_rca_lookup.xlsx")
COUNTRY_FILE = os.path.join(UTILS_DIR, "Updated CountryList.xlsx")
LOG_FILE = BASE_DIR / "be_gen_excels"/ "be_pep_gen.log"
...
def normalize_list_strings(
    ...
    For example '['belgium'],['uk']' -> 'belgium, UK' or ['belgium', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches belgium_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Belgium"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["be_gen"] * num_rows,
            "Updated On": [""] * num_rows,
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "BE"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match belgium_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches belgium_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    clean_df.to_excel(
        os.path.join(CLEANED_DIR, "pep_belgium_living_relevant_cleaned.xlsx"),
        index=False,
    )
...
def belgium_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    global RAW_FILE_PATH                    # the variable declared at top of file 
    if raw_file_path is not None: 
        RAW_FILE_PATH = raw_file_path 
    try:
        logger.info("Starting belgium PEP scraper...")
        ...
        logger.error(
            get_standard_logger_message(
                "belgium_pep_scrapper()", e, "Error in belgium PEP scraper"
            )
        )
        raise
