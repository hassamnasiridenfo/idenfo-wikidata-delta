...
METADATA = {
    "Name": "Full List Name Here",  # e.g., "PEP Kingdom of the Netherlands"
    "Publisher": "Publisher/Authority Name",  # e.g., "Dutch Government"
    "Description": "Detailed description",  # e.g., "Politically Exposed Persons from Netherlands"
    "Country": "Country Name",  # e.g., "Netherlands" or "Global"
    "Weblink": "https://source-url.com",  # Source website URL
    "Tag": "unique_tag_lowercase",  # e.g., "netherlands_pep" (lowercase, underscores)
    "Status": 1,  # 1 = Active (will run), 0 = Inactive (will skip)
}
...
def normalize_list_strings(
    ...
    For example '['nepal'],['uk']' -> 'nepal, UK' or ['nepal', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches nepal_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Nepal"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["np_gen"] * num_rows,
            "Updated On": [""] * num_rows,
            "Added On": ["2026-02-13"] * num_rows,
            "Status": [1] * num_rows,
            "Charges": [""] * num_rows,
...
def get_gender(genderLabel: str) -> str:
    ...
    if genders[0] in ["trans woman", "third gender", "cisgender man", "non-binary"]:
        return "Other"
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "NP"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match nepal_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches nepal_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    clean_df.to_excel(
        os.path.join(CLEANED_DIR, "pep_nepal_living_relevant_cleaned.xlsx"),
        index=False,
    )
...
def nepal_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(
            get_standard_logger_message(
                "nepal_pep_scrapper()", e, "Error in nepal PEP scraper"
            )
        )
        raise