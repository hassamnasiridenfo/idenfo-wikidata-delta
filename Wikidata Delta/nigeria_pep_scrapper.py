...
RAW_DIR = BASE_DIR / "ng_gen_excels"
CLEANED_DIR = BASE_DIR / "ng_gen_excels"
UTILS_DIR = BASE_DIR
...
#  PATHS
RAW_FILE_PATH     = os.path.join(RAW_DIR, "pep_nigeria_living_relevant_raw.xlsx")
CLEAN_XLSX   = os.path.join(CLEANED_DIR, "pep_nigeria_living_relevant_clean.xlsx")
RCA_FILE_PATH = os.path.join(CLEANED_DIR, "pep_nigeria_living_relevant_rca_lookup.xlsx")
COUNTRY_FILE = os.path.join(UTILS_DIR, "Updated CountryList.xlsx")
LOG_FILE = BASE_DIR / "ng_gen_excels"/ "ng_pep_gen.log"
...
TRANSLATION_CACHE_PATH = os.path.join(CLEANED_DIR, "pep_nigeria_translation_cache.xlsx")
...
def normalize_list_strings(
    ...
    For example '['nigeria'],['uk']' -> 'nigeria, UK' or ['nigeria', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches nigeria_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General Nigeria"] * num_rows,
            "List Category": [""] * num_rows,
            "List Type": ["Individual"] * num_rows,
            "Image Tag": [""] * num_rows,
            "Scraper Tag": ["ng_gen"] * num_rows,
            "Updated On": [""] * num_rows,
        ...
        designations.extend(
            [
                (
                    ...
                    and "military dictatorship" not in party.lower()
                    else party.title()
                )
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "NG"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match nigeria_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches nigeria_pep_scrapper.py"""
...
def get_clean_df() -> pd.DataFrame:
    ...
    if rca_records:
        ...
        clean_df.to_excel(
            os.path.join(CLEANED_DIR, "pep_nigeria_living_relevant_cleaned.xlsx"),
            index=False,
        )
...
def nigeria_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    ...
    try: 
        logger.info('Starting nigeria PEP scraper...') 

        clean_df = get_clean_df()
        clean_df = common_cleaning(clean_df)
        clean_df = replacements_for_delta(clean_df)
        _save_translation_cache()
        logger.info("nigeria PEP scraper completed successfully.")
        return clean_df
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "nigeria_pep_scrapper()", e, "Error in nigeria PEP scraper"
            )
        )
        raise
