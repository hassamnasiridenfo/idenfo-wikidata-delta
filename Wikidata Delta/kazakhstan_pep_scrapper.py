...
CLEANED_DIR = os.path.join(BASE_DIR, "kz_gen_excels")
RAW_DIR = os.path.join(BASE_DIR, "kz_gen_excels")
os.makedirs(CLEANED_DIR, exist_ok=True)
os.makedirs(RAW_DIR, exist_ok=True)
CLEAN_FILE_PATH = os.path.join(
    CLEANED_DIR, "pep_kazakhstan_living_relevant_cleaned.xlsx"
)
RAW_FILE_PATH = os.path.join(RAW_DIR, "pep_kazakhstan_living_relevant_raw.xlsx")
RCA_FILE_PATH = os.path.join(
    CLEANED_DIR, "pep_kazakhstan_living_relevant_rca_lookup.xlsx"
)

logger = logging.getLogger("KazakhstanPEPScrapper")
if not logger.hasHandlers():
    logger.setLevel(logging.INFO)
    handler = logging.FileHandler(os.path.join(CLEANED_DIR, "kazakhstan_pep.log"))
...
def normalize_list_strings(
    ...
    For example '['kazakhstan'],['uk']' -> 'kazakhstan, UK' or ['kazakhstan', 'UK'] BASED ON return_type
...
def format_name_value(value: object) -> str | None:
    """Format name as bracketed JSON list - matches kazakhstan_pep_scrapper.py"""
    if is_missing_value(value):
        return None
...
def initalize_clean_df(raw_df_len: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        data: dict = {
            ...
            "Category": ["General kazakhstan"] * num_rows,
...
def get_sheet_df(sheet: str) -> pd.DataFrame:
    try:
        ...
        logger.error(
            get_standard_logger_message(
                "get_sheet_df()", e, f"Raw file not found: {RAW_FILE_PATH}"
            )
        )
        raise e
        ...
        logger.error(
            get_standard_logger_message(
                "get_sheet_df()", e, f"Sheet '{sheet}' not found in raw file"
            )
        )
        return pd.DataFrame()


def get_raw_df(sheet: str = None) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def get_place_of_birth(birthPlaceLabel: str, extra_info: dict) -> str:
    ...
    if birthPlaceLabel.count("]") == 1 and birthPlaceLabel.count("[") == 1:
        ...
        if place.isascii():
            return place
        translated = translator.translate(place)
        return unidecode(translated) if translated else place
    else:
        ...
        if len(places) > 1:
            for place in places:
                if place.isascii():
                    ...
                    extra_info["POB in Other Language"] = unidecode(
                        translator.translate(place)
                    )

    if not pob:
        if extra_info["Extra POB"]:
            pob = extra_info["Extra POB"]
        elif extra_info["POB in Other Language"]:
            pob = extra_info["POB in Other Language"]
        else:
            pob = ""

    return pob if pob else ""
...
def get_role_details(
    ...
    if pd.notna(positionLabel) and positionLabel:
        ...
        url_pattern = r"https?:\/\/"
        designations.extend(
            [pos.title() for pos in positions if not re.search(url_pattern, pos)]
        )
    if pd.notna(politicalPartyLabel) and politicalPartyLabel:
        ...
        # remove any parties that match wikidata ID pattern
        parties = [p for p in parties if not re.match(wikidata_id_pattern, p)]
        designations.extend(
            [
                (
                    f"Member of {party}"
...
def clean_name(name_data: dict) -> dict:
    ...
    prefix_matches = r"\bDr\.?\b|\bMr\b|\bMrs\b"

    matches = re.findall(prefix_matches, name_str)

    if matches:
        name_data["aliases"].append(clean_alias(name_str))
        name_data["alias_types"].append(get_alias_type(name_str))
        name_str = re.sub(prefix_matches, "", name_str).strip()
...
def get_father_name(fatherLabel: str) -> str:
    ...
    # check if name is wikidata ID, if so return empty
    ...
    for s in strs_to_rem:
        father_name = father_name.replace(s, "").strip()

    father_name = father_name.replace("-", " ").replace("&", " and ").strip()
...
def load_rca_lookup() -> tuple[dict[str, str], pd.DataFrame | None]:
    ...
    try:
        df = pd.read_excel(
            os.path.join(CLEANED_DIR, "pep_kazakhstan_living_relevant_rca_lookup.xlsx")
        )
    except Exception:
...
def build_rca_rows(
    rca_df: pd.DataFrame, existing_lookup: dict[str, str], country_code: str = "KZ"
) -> tuple[list[dict], dict[str, list[tuple[str, str]]]]:
    """
    Build RCA rows and track parent-child relationships.
    CORRECTED to match kazakhstan_pep_scrapper.py logic exactly.
...
def save_rca_lookup(rca_records: list[dict], existing_df: pd.DataFrame | None) -> None:
    """Save RCA lookup workbook - matches kazakhstan_pep_scrapper.py"""
    lookup_path = RCA_FILE_PATH
...
def get_clean_df() -> pd.DataFrame:
    ...
    for i, row in raw_df.iterrows():
        try:
            ...
            clean_df.at[i, "Extra Information"] = json.dumps(
                extra_info, ensure_ascii=True
            )
        ...
        for col in clean_df.columns:
            if col not in rca_df_clean.columns:
                if col in list_columns:
                    ...
                    rca_df_clean[col] = "General kazakhstan"
...
def kazakhstan_pep_scrapper(raw_file_path: str = None) -> pd.DataFrame: 
    global RAW_FILE_PATH                    # the variable declared at top of file 
    if raw_file_path is not None: 
        RAW_FILE_PATH = raw_file_path       # override with the path passed in 
    try: 
        logger.info('Starting kazakhstan PEP scraper...') 

        clean_df = get_clean_df()  
        clean_df = common_cleaning(clean_df)
        clean_df = replacements_for_delta(clean_df)
        logger.info("kazakhstan PEP scraper completed successfully.")
        return clean_df
    except Exception as e:
        logger.error(
            get_standard_logger_message(
                "kazakhstan_pep_scrapper()", e, "Error in kazakhstan PEP scraper"
            )
        )
        raise
