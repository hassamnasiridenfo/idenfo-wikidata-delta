...
def _collect_party_designations(pep_role: pd.DataFrame) -> dict[str, list[str]]:
    ...
    if pep_role.empty or "ID" not in pep_role.columns or "politicalPartyLabel" not in pep_role.columns:
        return collected
    ...
    for identifier, raw_party in zip(subset["ID"], subset["politicalPartyLabel"], strict=True):
        ...
        if not party_text:
            continue
        entry = party_text if _should_skip_party_prefix(party_text) else f"Member of {party_text}"
...
def _extract_ascii_initial(token: str) -> str:
    ...
    ascii_letters = [character for character in stripped if character.isalpha() and character.isascii()]
    if ascii_letters:
...
def _extract_relation_entities(value: str) -> list[str]:
    ...
    """
    matches = [match.strip() for match in re.findall(r'\["([^\"]+)"\]', value) if match.strip()]
    if matches:
        return matches
...
def _collect_parent_entities(subset: pd.DataFrame) -> list[tuple[str, str]]:
    ...
    for pep_id, relation_value in subset.itertuples(index=False):
        ...
        entities = _extract_relation_entities(str(relation_value)) or [str(relation_value)]
    ...
    for _, row in pep_alias.iterrows():
        ...
        for column, default_alias_type in allowed_columns.items():
            ...
            for alias_value, had_double_quotes in alias_entries:
                if not alias_value:
                    continue
                alias_type = "Also Known As" if had_double_quotes else default_alias_type
                if alias_type != "Original Script Name" and _is_original_script_alias(alias_value):
                    alias_type = "Original Script Name"
                _register_alias_entry(collected_values, collected_types, alias_value, alias_type)
...
def _build_alias_columns(
    ...
    if "Original Script Name" in ireland_df.columns:
        ...
        original_script_series = pd.Series([None] * len(ireland_df), index=ireland_df.index)
    ...
    for row_index, identifier in enumerate(ireland_df["ID"]):
        ...
        if not pd.isna(identifier):
            ...
            for alias_value, alias_type in base_aliases:
                _register_alias_entry(alias_entries, alias_types, alias_value, alias_type)

        original_script_entries = _normalise_relation_entries(original_script_series.iloc[row_index])
        for entry in original_script_entries:
            _register_alias_entry(alias_entries, alias_types, entry, "Original Script Name")
...
def _load_existing_rca_lookup(output_path: Path) -> tuple[dict[str, str], pd.DataFrame | None]:
    ...
    try:
        ...
        LOGGER.warning("⚠️ Unable to read existing RCA lookup at %s", lookup_path)
        return {}, None
    ...
    if not name_column or not id_column:
        LOGGER.warning(
            "⚠️ RCA lookup at %s is missing required columns", lookup_path,
        )
        return {}, pd.DataFrame(columns=["RCA Name", "RCA ID"])
...
def _get_or_create_rca_identifier(
    ...
    flattened_name = _flatten_bracketed_value(formatted_name) if formatted_name else None
    ...
    identifier = (
        f"IE-GEN-{initials}-{combo}-RCA-"
        f"{sequence_tracker[tracker_key]}"
    )
...
def _fill_missing_structured_entries(ireland_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in updated.columns:
        ...
        placeholder = (
            _detect_placeholder(sample_value)
        ...
        updated[column] = updated[column].apply(
            lambda value, placeholder=placeholder: placeholder
            if _needs_placeholder(value)
            else value,
        )
...
def _merge_main_sheet(ireland_df: pd.DataFrame, pep_main: pd.DataFrame) -> pd.DataFrame:
    ...
    if pep_main.empty:
        return ireland_df
    existing_main_columns = [column for column in main_columns if column in pep_main.columns]
...
def _apply_column_mappings(
    ...
    column_mappings = {
        ...
        "Primary Address": _map_single_value(pep_address,"ID","residenceLabel",
    ...
    for column_name in ("Start Date", "End Date"):
        if column_name in ireland_df.columns:
            ireland_df[column_name] = ireland_df[column_name].apply(_format_role_date_value)
...
def _normalise_image_tag_value(raw_value: object) -> str | None:
    ...
    if isinstance(raw_value, str):
        ...
        hyperlink_match = re.match(r'=HYPERLINK\("([^"]+)"', trimmed, flags=re.IGNORECASE)
        if hyperlink_match:
...
def _country_title_to_nationality_map() -> dict[str, str]:
    ...
    """
    country_frame = _load_updated_country_list()
    if country_frame is None:
        return {}

    available_title_columns = [
        column for column in COUNTRY_LIST_TITLE_COLUMNS if column in country_frame.columns
    ]
...
def _collect_prefixed_entries_for_frame(
    ...
    for prefix, candidates in prefix_config:
        ...
        for identifier, raw_value in zip(identifiers, frame[actual_column], strict=True):
...
def _append_rca_ids_to_sources(
    ...
    for source_id, rca_entries in source_to_rca_links.items():
        filtered_entries = [
            ...
            if isinstance(entry[0], str) and entry[0].lower() not in EXCLUDED_RELATIONSHIP_COLUMNS
        ]
        ...
        updated.loc[mask, "Relationship Type"] = updated.loc[mask, "Relationship Type"].apply(
...
def _ensure_all_columns(ireland_df: pd.DataFrame, ireland_cols: pd.Index) -> pd.DataFrame:
...
def pep_to_ireland44(
    pep_file: str | Path,
    ireland_template_file: str | Path,
    ...
    ireland_cols = pd.read_excel(template_path, sheet_name="Sheet1").columns

    unique_ids = _collect_unique_ids(
        [pep_main, pep_dob, pep_nat, pep_alias, pep_address, pep_case, pep_role, pep_rca],
    )
    ...
    ireland_df.to_excel(output_path, index=False, sheet_name="Ireland 44")
    LOGGER.info(
        "✅ All PEP sheets collated into a single tab with Ireland 44 formatting. Saved to: %s",
        output_path,
    )
...
if __name__ == "__main__":
    ...
    pep_to_ireland44(
        "pep_ireland_living_relevant_20251107_161429.xlsx",
        "Sample Ireland 44.xlsx",
    )
